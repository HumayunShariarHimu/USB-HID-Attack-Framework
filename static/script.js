...
async function runDuckyScript(script) {
    ...
    try {
        for (const line of lines) {
            const t = line.trim();
            if (!t || t.startsWith('REM') || t.startsWith('//')) continue;
            const parts = t.split(' ');
            const cmd = parts[0].toUpperCase();
            const arg = parts.slice(1).join(' ');
            switch (cmd) {
                case 'STRING': await typeString(arg); break;
                case 'DELAY': await sleep(parseInt(arg) || 200); break;
                case 'ENTER': await sendHIDReport(0, [SPECIAL['ENTER']]); await sleep(20); break;
                case 'TAB': await sendHIDReport(0, [SPECIAL['TAB']]); await sleep(20); break;
                case 'BACKSPACE': await sendHIDReport(0, [SPECIAL['BACKSPACE']]); await sleep(20); break;
                case 'ESC': await sendHIDReport(0, [SPECIAL['ESC']]); await sleep(20); break;
                case 'GUI': case 'WINDOWS': await sendHIDReport(0x08, []); await sleep(50); await sendHIDReport(0x00, []); break;
                case 'CTRL': await sendHIDReport(0x01, []); await sleep(50); await sendHIDReport(0x00, []); break;
                case 'SHIFT': await sendHIDReport(0x02, []); await sleep(50); await sendHIDReport(0x00, []); break;
